...
import { Search, ArrowRight, Upload as UploadIcon, Zap, Sparkles, Package, Users } from "lucide-react";
...
export default function Home() {
  ...
  const [globalStats, setGlobalStats] = useState({ assets: 0, creators: 0, downloads: 0 });
  ...
  const loadAll = async () => {
    try {
      ...
      setGlobalStats({ assets: totalAssets, creators: cr.data.length, downloads: totalDl });
    } catch (e) { /* noop */ }
  };
  ...
  const handleSearchSubmit = (e) => { e.preventDefault(); if (q.trim()) navigate(`/explore?q=${encodeURIComponent(q)}`); };
  const hasData = cats.some((c) => c.count > 0);

  return (
    <div className="relative">
      {/* HERO */}
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-24">
        <div className="orb w-[520px] h-[520px] top-10 -left-40 bg-[#00E5FF]/30 animate-pulse-slow" />
        <div className="orb w-[600px] h-[600px] -top-20 -right-40 bg-[#7C3AED]/30 animate-pulse-slow" />
        <div className="orb w-[420px] h-[420px] bottom-0 left-1/3 bg-[#009DFF]/25" />
        <div className="absolute inset-0 grid-bg" />

        <motion.div style={{ y: heroY, scale: heroScale, opacity: heroOpacity }} className="relative z-10 max-w-5xl mx-auto text-center px-6">
          <motion.img
            src="/lazr-logo.png"
            alt="LazR Hub"
            initial={{ scale: 0.5, opacity: 0, rotate: -10 }}
            animate={{ scale: [1, 1.05, 1], opacity: 1, rotate: 0 }}
            transition={{ scale: { duration: 4, repeat: Infinity, ease: "easeInOut" }, opacity: { duration: 1 }, rotate: { duration: 1 } }}
            className="w-32 h-32 md:w-40 md:h-40 mx-auto object-contain mb-6 drop-shadow-[0_0_60px_rgba(0,229,255,0.6)]"
          />

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            className="text-[10px] tracking-[0.4em] uppercase text-[#00E5FF] font-heading mb-4">
            Digital Creator Hub
          </motion.div>
          ...
          >
            <motion.span className="block" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>Discover.</motion.span>
            <motion.span className="block gradient-text" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}>Create.</motion.span>
            <motion.span className="block" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }}>Share.</motion.span>
          </motion.h1>

          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }}
            className="mt-8 text-base md:text-lg text-[#B8C2CC] max-w-2xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}
            ...
            {[
              { n: fmtNum(globalStats.assets), l: "Assets" },
              { n: fmtNum(globalStats.creators), l: "Creators" },
              { n: fmtNum(globalStats.downloads), l: "Downloads" },
            ].map((s, i) => (
          ...
          <div className="flex gap-2 mt-4 flex-wrap justify-center">
            {SUGGESTIONS.map((s) => (
              <button type="button" key={s} onClick={() => setQ(s)}
                ...
                data-testid={`search-chip-${s}`}>{s}</button>
            ))}
          </div>
        </form>
        {searchResults && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 glass-strong rounded-2xl p-4">
            <div className="text-xs tracking-[0.2em] uppercase text-[#00E5FF] font-heading mb-3">
              {searchResults.length} live results
            </div>
            {searchResults.length === 0 ? (
              <div className="text-center py-8">
                <div className="font-heading text-white mb-1">No assets found.</div>
                <div className="text-sm text-[#B8C2CC]/70">Try searching something different.</div>
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {searchResults.slice(0, 8).map((a) => (
                  <Link to={`/asset/${a.asset_id}`} key={a.asset_id} className="group flex gap-3 items-center p-2 rounded-lg hover:bg-white/5 transition">
                    <img src={resolveUrl(a.preview_url)} className="w-12 h-12 rounded-lg object-cover" alt="" loading="lazy" />
      ...
      <section className="relative py-24 px-6 md:px-12 max-w-7xl mx-auto">
        ...
        {featured.length === 0 ? (
          <EmptyState
            ...
            subtitle="Be the first creator to upload an asset and set the tone for LazR Hub."
            action={<Link to="/upload" className="btn-primary mt-4"><UploadIcon size={14} /> Upload Your First Asset</Link>}
          />
        ) : (
      ...
      {trending.length > 0 && (
        <section className="relative py-24">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="px-6 md:px-12 max-w-7xl mx-auto mb-10">
              ...
              {[...trending, ...trending].map((a, i) => (
                <Link to={`/asset/${a.asset_id}`} key={i} className="relative w-72 shrink-0 card-lazr overflow-hidden cursor-hover">
                  <div className="aspect-[4/3] overflow-hidden">
                    <img src={resolveUrl(a.preview_url)} className="w-full h-full object-cover" alt="" loading="lazy" />
                  </div>
                  <div className="p-4">
      ...
      <section className="relative py-24 px-6 md:px-12 max-w-7xl mx-auto">
        ...
        {creators.length === 0 ? (
          <EmptyState
            ...
            action={<Link to="/register" className="btn-primary mt-4"><Sparkles size={14} /> Create your account</Link>}
          />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {creators.map((cr, i) => (
